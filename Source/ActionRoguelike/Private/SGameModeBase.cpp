...
void ASGameModeBase::StartPlay()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("PowerupClass over 0"));

		// Run EQS to find potential power-up spawn locations
		UEnvQueryInstanceBlueprintWrapper* QueryInstance = UEnvQueryManager::RunEQSQuery(this, PowerupSpawnQuery, this, EEnvQueryRunMode::AllMatching, nullptr);
		if (ensure(QueryInstance))
		{
			UE_LOG(LogTemp, Warning, TEXT("Powerup Query Run"));
...
void ASGameModeBase::OnMonsterLoaded(FPrimaryAssetId LoadedId, FVector SpawnLocation)
{
	...
	if (Manager)
	{
		...
		if (MonsterData)
		{
			AActor* NewBot = GetWorld()->SpawnActor<AActor>(MonsterData->MonsterClass, SpawnLocation, FRotator::ZeroRotator);//Use successful EQS query and spawn MinionClass
			if (NewBot)
			{
